 ...
 * 虚拟腿长变化速度 */
std::tuple<float, float, float, float> VMCsolve(
  float phi1, float phi4,float eulrPit,float d_eulrPit,float omega1,float omega4,float dt) {
  ...
  this->vmc_leg_.phi2 =
      2 *
      atan2f((this->vmc_leg_.B0 + sqrtf(this->vmc_leg_.A0 * this->vmc_leg_.A0 +
  ...
  this->vmc_leg_.phi0 = atan2f(
      this->vmc_leg_.YC, (this->vmc_leg_.XC - this->param_.hip_length / 2.0f));
  ...
  vmc_leg_.x_length = 0.202f*sqrtf(1.0f - powf((powf(vmc_leg_.n_length,2)+0.040804f-0.0025f)/(0.404f*vmc_leg_.n_length),2));
  ...
  float d_phi2 = fabsf(sin_p2_p3) > 1e-6f ?
    (-this->param_.leg_1 * sinf(phi1 - this->vmc_leg_.phi3) * omega1
     + this->param_.leg_4 * sinf(phi4 - this->vmc_leg_.phi3) * omega4)
    / (this->param_.leg_2 * sin_p2_p3) : 0.0f;
  float dXC = -this->param_.leg_1 * sinf(phi1) * omega1 - this->param_.leg_2 * sinf(this->vmc_leg_.phi2) * d_phi2;
  float dYC =  this->param_.leg_1 * cosf(phi1) * omega1 + this->param_.leg_2 * cosf(this->vmc_leg_.phi2) * d_phi2;
  /* 腿长变化速度得出于雅可比 效果没有角速度反馈几何计算好 */
  this->vmc_leg_.d_L0 = ((this->vmc_leg_.XC - this->param_.hip_length / 2.0f) * dXC + this->vmc_leg_.YC * dYC) / this->vmc_leg_.L0;
...
std::tuple<float, float>VMCinserve(
  float phi1, float phi4, float Tp,float F0) {
  /*jacobian矩阵计算*/
  this->vmc_leg_.j11 =
...
float GndDetector(
  float T1, float T2, float imu_accl_z, float theta, float d_theta,float dt) {
...
/* 角速度变化和腿长变化乘积太大了 有点失真*/
  // vmc_leg_.Fn =LowpassFilter(std::clamp(vmc_leg_.F * cosf(theta) + vmc_leg_.Tp * sinf(theta) / vmc_leg_.L0 +
  //              5.0f +  (imu_accl_z * 9.8f  + 2.0f * vmc_leg_.d_L0 * d_theta * sinf(theta) +
  //                     vmc_leg_.L0 * d_theta * d_theta * cosf(theta)),-200.0f,200.0f),100.0f,dt);
