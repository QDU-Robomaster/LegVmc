...
std::tuple<float, float, float, float> VMCsolve(
  float phi1, float phi4,float eulrPit,float d_eulrPit,float dt) {
  ...
  this->vmc_leg_.d_phi0 = (this->vmc_leg_.phi0 - this->vmc_leg_.last_phi0) / dt;
  ...
  this->vmc_leg_.dd_L0 = (this->vmc_leg_.d_L0 - this->vmc_leg_.last_d_L0) / dt;
...
float GndDetector(
  float T1, float T2, float imu_accl_z, float theta, float d_theta) {
